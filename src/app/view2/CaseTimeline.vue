<template>

  <div class="timeCase">
    <div class="showTimeLine">
      <Timeline>
        <TimelineItem v-for="(value,key) in timeLine" :key="key" @click.native="changeGraph(key,value)" :color="(key === tiemCircle ? 'red' : 'blue')">
              <!-- <p class="time" :style="{'color': (key === tiemCircle ? 'red' : 'black'),'fontSize':(key === tiemCircle ? '20px' : '16px')}">{{value.date}}</p> -->
              <p :class="key == tiemCircle ? 'time timeActive' : 'time'">{{value.date}}</p>
          </TimelineItem>
    </Timeline>
    <!-- <button @click="exportData">export</button> -->
    </div>

    <div class="showGraph">
      <svg id="topo" width="1700" height="1020">
        <g class="graphContent" transform="translate(5,30)">
          <defs>
            <marker id="arrowhead" viewBox="-0 -5 10 10" refX="23" refY="0" orient="auto" markerWidth="13" markerHeight="13" xoverflow="visible">
              <path d="M 0,-5 L 10 ,0 L 0,5" fill="#999" style="stroke: none;"></path>
            </marker>
          </defs>
          
        </g>
      </svg>
    </div>

    <div class="tooltip">
      <p v-for="(item,key) in tooltip_value" v-show="item!==''" :key="key">
        <b>{{ key }} : </b>
        <span :class="key">{{ item }}</span>
      </p>
    </div>
  </div>
</template>

<script>
import ip_timeline from '../../static/demo_data/nnnn.json'
import FileSaver from "file-saver";
export default {
  data () {
    return {
     nodes:[],
     links:[],
     graphNodes:[],
     graphLinks:[],
     svgLinks:'',
     svgNodes:'',
     svgEdgepaths:'',
     svgEdgelabels:'',
     svgNodeText:'',
     simulation:'',
     timeLine:[],
     tiemCircle:59,
     dateColor:0,
     tooltip_value:{
        'name':'name',
        'type':'domain'
      },
     node_types: [
        "Domain",
        "IP",
        "Cert_SHA256",
        "IP_CIDR",
        "ASN",
        "Whois_Name",
        "Whois_Email",
        "Whois_Phone",
      ],
      link_types: [
        "r_dns_a",
        "r_dns_cname",
        "r_cert",
        "r_cidr",
        "r_asn",
        "r_whois_name",
        "r_whois_email",
        "r_whois_phone",
        "r_subdomain",
        "r_certchain",
        "r_request_jump",
      ],
    }
  },
  methods:{
    nodeColorScale(val) {
      // 自定义节点的颜色
      const nodeColorScale = d3
        .scaleOrdinal()
        .domain(this.node_types)
        .range([
          "#ff9e6d",
          "#86cbff",
          "#c2e5a0",
          "#fff686",
          "#9e79db",
          "#8dd3c7",
          "aquamarine",
          "aqua",
        ]);
      return nodeColorScale(val);
    },

    linkColorScale(val) {
      // 自定义连边的颜色
      const linkColorScale = d3
        .scaleOrdinal()
        .domain(this.link_types)
        .range([
          "#ff9e6d",
          "#86cbff",
          "#c2e5a0",
          "#fff686",
          "#9e79db",
          "#8dd3c7",
          "aquamarine",
          "aqua",
          "crimson",
          "#009966",
          '#006633'
        ]);
      return linkColorScale(val);
    },
     createGraph(nodes, edges) {
      const that = this;
      const width = 1300, height = 930; // SVG的大小
      const margin = {
        // 四周的边距
        top: 30,
        right: 80,
        bottom: 5,
        left: 5,
      };

      let svg = d3.select("#topo") // 选中SVG
                  .call(d3.zoom().scaleExtent([-5, 2]).on("zoom", zoom_action)); // 添加平移和缩放功能

      let content = d3.select('.graphContent');

      /*----------初始化连边------------------------*/
      let all_links = content.append('g').attr('class','allLinks');
      this.svgEdgepaths = all_links.selectAll(".edgepath") //连边上的标签位置,是的文字按照这个位置进行布局
						.data(edges,d=>d.id)
						.enter()
						.append('path')
						.attr('class', 'edgepath')
						.attr("stroke",d => this.linkColorScale(d.relation))
						.attr('id', function (d, i) {return 'edgepath' + d.id})
            .style("pointer-events", "none")
            .attr('relation',d=>d.relation)
            .attr('marker-end','url(#arrowhead)');

      this.edgelabels = all_links.selectAll(".edgelabel")
						.data(edges,d=>d.id)
						.enter()
						.append('text')
						.style("pointer-events", "none")
						.attr('class', 'edgelabel')
						.attr('id', function (d, i) {return 'edgelabel' + i})
            .attr('font-size', 12)
						.attr('fill', d => this.linkColorScale(d.relation));

      this.edgelabels
        .append("textPath") //要沿着<path>的形状呈现文本，请将文本包含在<textPath>元素中，该元素具有一个href属性，该属性具有对<path>元素的引用.
        .attr("xlink:href", function (d, i) {
          return "#edgepath" + d.id;
        })
        .style("text-anchor", "middle")
        .style("pointer-events", "none")
        .attr("startOffset", "50%")
        .text((d) => d.relation);

      /*----------------------初始化节点-----------------------------*/
      let all_nodes = content.append('g').attr('class','allNodes');

      this.svgNodes = all_nodes
        .selectAll(".nodes")
        .data(nodes, d=>d.id)
        .enter()
        .append("circle")
        .attr("r", (d) => 17)
        .style("stroke", "grey")
        .style("stroke-opacity", 0.3)
        .style("fill", (d) => this.nodeColorScale(d.type))
        .attr("class", "nodes")
        .attr("id", (d) => d.id)
        .call(
          d3.drag()
            .on("start", that.dragstarted)
            .on("drag", that.dragged)
            .on("end", that.dragended)
        );
       
       this.svgNodeText = all_nodes
        .selectAll('.nodeText')
        .data(nodes,d=>d.id)
        .enter()
        .append("text")
        .attr("dy", 4)
        .attr("dx", -15)
        .text((d) => d.name);

      /*---------------------定义力导引模型----------------------*/
      this.simulation = d3.forceSimulation();

      this.simulation.nodes(nodes).on("tick", ticked);

      this.simulation
        .force("link", d3.forceLink())
        .force("charge", d3.forceManyBody())
        .force("center", d3.forceCenter())
        // .force('collide',d3.forceCollide().radius(30).iterations(2));

      this.simulation.force("link").id(function (d) {
          return d.name;
        })
        .distance(100)
        .links(edges);
      this.simulation.force("charge").strength(-50).distanceMax(200);
      this.simulation.force("center").x(width / 2).y(height / 2);

      /*---------------------自定义函数，用于图形缩放和力导引模型-------------*/

      function zoom_action() {
        // 控制图形的平移和缩放
        content.attr("transform", d3.event.transform);
      }

      function ticked() {
        that.svgEdgepaths.attr("d", (d) => "M " +d.source.x +" " +d.source.y +" L " +d.target.x +" " +d.target.y);   
        that.svgNodes.attr("cx", d=>d.x).attr("cy", d=>d.y);

        that.svgNodeText.attr("x", d=>d.x).attr("y", d=>d.y);
      }
    },
    dragstarted(d) {
        if (!d3.event.active) this.simulation.alphaTarget(0.3).restart();
        d.fy = d.y;
        d.fx = d.x;
    },
    dragged(d) {
        d.fx = d3.event.x;
        d.fy = d3.event.y;
      },
    dragended(d) {
        if (!d3.event.active) {
          this.simulation.alphaTarget(0);
        }
        d.fx = null;
        d.fy = null;
    },
    changeGraph(key,value){
      // if(key > 11){
      //   d3.select('.showTimeLine')._groups[0][0].scrollTop += 47;
      // }
      let that = this;
      this.tiemCircle = key;
      let timeEnd = value.date;
      
      let rawNodes = JSON.parse(JSON.stringify(Object.values(ip_timeline.nodes)));
      let rawLinks = JSON.parse(JSON.stringify(Object.values(ip_timeline.links)));
      let newNode = rawNodes.filter(d => (new Date(d.start) < new Date(timeEnd) || d.start === timeEnd) && (new Date(d.end) > new Date(timeEnd) || d.end === timeEnd));
      let nodes = []
      let edges = rawLinks.filter(d => (new Date(d.start) < new Date(timeEnd) || d.start === timeEnd) && (new Date(d.end) > new Date(timeEnd) || d.end === timeEnd));

    for(let node_info of newNode){
      let nodeFlag = 1;
      for(let currentNode of this.graphNodes){
          if(node_info.name == currentNode.name){
            nodes.push(currentNode)
            nodeFlag = 0;
          }
        }
      if(nodeFlag){
        nodes.push(node_info)
      }
    }
      
      let newobj = {}; 
      nodes = nodes .reduce((preVal, curVal) => {
          newobj[curVal.name] ? '' : newobj[curVal.name] = preVal.push(curVal); 
          return preVal 
      }, [])

      let newobjLink = {}; 
      edges = edges .reduce((preVal, curVal) => {
          newobjLink[curVal.id] ? '' : newobjLink[curVal.id] = preVal.push(curVal); 
          return preVal 
      }, [])

      this.svgEdgepaths = this.svgEdgepaths.data(edges,d=>d.id)
      this.svgEdgepaths.exit().remove();
      this.svgEdgepaths = this.svgEdgepaths
              .enter() 
              .append('path')
              .attr('class', 'edgepath')
              .attr("stroke",d => this.linkColorScale(d.relation))
              .attr('id', function (d, i) {return 'edgepath' + i})
              .style("pointer-events", "none")
              .attr('marker-end','url(#arrowhead)')
              .merge(this.svgEdgepaths);

      this.edgelabels = this.edgelabels.data(edges,d=>d.id);
      this.edgelabels.exit().remove();
      this.edgelabels = this.edgelabels
              .enter()
              .append('text')
              .style("pointer-events", "none")
              .attr('class', 'edgelabel')
              .attr('id', function (d, i) {return 'edgelabel' + i})
              .attr('font-size', 12)
              .attr('relation',d=>d.relation)
              .attr('fill', d => this.linkColorScale(d.relation))
              .merge(this.edgelabels);

      d3.selectAll('textPath').remove()
      this.edgelabels
          .append("textPath")
          .attr("xlink:href", function (d, i) {
            return "#edgepath" + d.id;
          })
          .style("text-anchor", "middle")
          .style("pointer-events", "none")
          .attr("startOffset", "50%")
          .text((d) => d.relation);
    // 连边data join end

    // 节点data join start
      this.svgNodes = this.svgNodes.data(nodes,d=>d.id);
      this.svgNodes.exit().remove();
      this.svgNodes = this.svgNodes
        .enter()
        .append("circle")
        .attr("r", (d) => 17)
        .style("stroke", "grey")
        .style("stroke-opacity", 0.3)
        .style("fill", (d) => this.nodeColorScale(d.type))
        .attr("class", "nodes")
        .attr("id", (d) => d.id)
        .call(
          d3.drag()
            .on("start", that.dragstarted)
            .on("drag", that.dragged)
            .on("end", that.dragended)
        )
        .merge(this.svgNodes);

      this.svgNodeText = this.svgNodeText.data(nodes,d=>d.id)
      this.svgNodeText.exit().remove();
      this.svgNodeText = this.svgNodeText
        .enter()
        .append("text")
        .attr("dy", 4)
        .attr("dx", -15)
        .text((d) => d.name)
        .merge(this.svgNodeText);
      // 节点data join end
      this.simulation.nodes(nodes).force("link").links(edges);
		  this.simulation.alpha(1).restart()
      // this.simulation.tick(10000)
      this.graphLinks = edges;
      this.graphNodes = nodes;
    },
    exportData() {
      let domains = [];
      for (let value of this.nodes){
        if(value.type == 'Domain'){
          domains.push(value.name)
        }
      }
      let data = JSON.stringify({ nodes: domains});
      let blob = new Blob([data], { type: "text/json" });
      FileSaver.saveAs(blob, "data.json");
    },
  },
  mounted(){
    let that = this;
    this.nodes = JSON.parse(JSON.stringify(Object.values(ip_timeline.nodes)));
    this.links = JSON.parse(JSON.stringify(Object.values(ip_timeline.links)));
    let dateArr = [];
    for(let value of this.nodes){
        dateArr.push(value.start);
        dateArr.push(value.end);
    }
    this.timeLine = Array.from(new Set(dateArr)).sort().map((d,i)=>{return {'date':d,'index':i}});
    let timeStart ='2020-08-15';
    let timeEnd = '2014-08-07';
    console.log(this.timeLine.length)
    this.graphNodes = this.nodes.filter(d => (new Date(d.start) < new Date(timeEnd) || d.start === timeEnd) && (new Date(d.end) > new Date(timeEnd) || d.end === timeEnd));
    this.graphLinks = this.links.filter(d => (new Date(d.start) < new Date(timeEnd) || d.start === timeEnd) && (new Date(d.end) > new Date(timeEnd) || d.end === timeEnd));
    this.createGraph(this.graphNodes,this.graphLinks);
    let currentLoop = 59;
    console.log(this.graphNodes,this.graphLinks)
    let time = setInterval(function(){
      if(currentLoop == that.timeLine.length){
        clearInterval(time)
      }
      if (currentLoop == 59) {
        console.log( d3.select('.showTimeLine'));
        d3.select('.showTimeLine')._groups[0][0].scrollTop = 47*currentLoop;
      }
        // d3.select('.showTimeLine')._groups[0][0].scrollTop += d3.select('.showTimeLine')._groups[0][0].scrollHeight / that.timeLine.length;
        // d3.select('.showTimeLine')._groups[0][0].scrollTop = 47*currentLoop;
        // console.log(`${that.timeLine[currentLoop].date} : ${d3.select('.showTimeLine')._groups[0][0].scrollTop}`)
        that.changeGraph(currentLoop,that.timeLine[currentLoop])
        currentLoop++; 
    },1000);

  }
}
</script>

<!-- 
  change domain : []

 -->
<style scoped>
.tooltip {
  position: absolute;
  visibility: hidden;
  padding: 10px 10px 10px 10px;
  opacity: 0.8;
  background-color: #515a6e;
  font-size: 14px;
  line-height: 30px;
  left: 0px;
  top: 0px;
  border-radius: 10px;
  color: aliceblue;
}
.showTimeLine{
  float: left;
  width: 150px;
  height: 965px;
  overflow:auto;
  margin: 30px 10px 0px 20px;
}
.showTimeLine::-webkit-scrollbar {
    display: none;
}
.showGraph{
  float: left;
  width: 1700px;
}
.time{
  font-size: 16px;
  font-weight: bold;
  color: black;
}
.timeActive{
  color: red;
  font-size: 20px;
  position: relative;
  top: -5px ;
}
</style>
