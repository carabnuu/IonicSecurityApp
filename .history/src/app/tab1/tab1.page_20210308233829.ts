...
export class Tab1Page {
    ...
    d3: any;
    constructor(private _platform: Platform,private screenshot:Screenshot) {
    ...
    shrink(){     
        this.svg.transition().duration(200).call(this.zoom.scaleBy, 0.75);
    }
    stripscript(s) {
    ...
    filternode(value){
        ...
        var temptarget=[];
         ...
         this.links.forEach(d => {
            if(d.source.name===value){
                templink.push(d)
                temptarget.push(d.target.name);
             }
             if(d.target.name===value){
                templink.push(d)
                tempsource.push(d.source.name);
             }
        });
         this.nodes.forEach(d => {
            if(d.name===value){
            ...
            if(tempsource.indexOf(d.name)>=0&&d.name!=value){
                tempnode.push(d)
            }
            if(temptarget.indexOf(d.name)>=0&&d.name!=value){
                tempnode.push(d)
            }
        }); 
        d3.selectAll(".Chart1svg").remove();
        this.init();
        this.drawChart(tempnode,templink)
        this.showcard();
    }
    svg:any;
    zoom:any;
    ...
    drawChart(nodes, edges) {
        ...
        */
        ...
        node.append("circle")
            ...
            .attr("stroke", "grey")
            .attr("id",d=>"n"+this.stripscript(d.name))
